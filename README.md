...
- **File attachments** (`files_write`): `+` by the composer (or drag-drop, or paste) uploads files into `<cwd>/uploads/` — any type dext can read from disk — and the prompt quotes their workspace paths; `⤓` fetches a URL host-side into the same folder (private addresses refused). Chips show image thumbnails and upload progress; images, PDFs (browser viewer), and text preview inline or open in a tab through the authenticated file endpoint.
