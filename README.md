...
- **File attachments** (`files_write`): `+` by the composer (or drag/drop/paste) uploads into `<cwd>/uploads/`; `⤓` safely fetches a public URL. Images (including AVIF/BMP/TIFF/HEIC), PDF, text/config data, and common Office/ODF documents can be served back with type-safe preview/download behavior. Paths and MIME hints are handed to dext for format-aware inspection. Native image understanding still requires Dext core image content blocks; until then vision-capable models receive only the workspace path and may use local OCR/tooling.
