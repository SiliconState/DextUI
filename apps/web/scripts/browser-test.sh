...
wait_js '(()=>{const g=[...document.querySelectorAll(`.activity[data-state="complete"]`)].find(x=>x.textContent.includes("screen sample.png"));return !!g&&g.textContent.includes("Inspected image")&&g.textContent.includes("image → context")&&!document.querySelector(`[data-agent-id="tool.vision_1"]`)})()' || { echo "FAIL: compact image outcome summary"; FAIL=1; }
...
# write_file, HTTP, read_image and commit get tasteful summaries without
# losing their exact detail/Raw path. Image keeps its meaningful outcome.
wait_js '(()=>{const a=[...document.querySelectorAll(`.activity[data-state="complete"]`)];return a.some(x=>x.textContent.includes("Wrote")&&x.textContent.includes("generated.ts"))&&a.some(x=>x.textContent.includes("Requested")&&x.textContent.includes("GET https://example.test")&&x.textContent.includes("HTTP 200 OK"))&&a.some(x=>x.textContent.includes("Inspected image")&&x.textContent.includes("image → context"))&&a.some(x=>x.textContent.includes("Committed")&&x.textContent.includes("Fold tool activity")&&x.textContent.includes("commit abc1234"))})()' || { echo "FAIL: special tool summaries"; FAIL=1; }
