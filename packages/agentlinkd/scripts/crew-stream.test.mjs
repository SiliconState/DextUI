...
test("authenticated host streams selected logs, replays reconnect cursors, and unsubscribes", { timeout: 20000 }, async (t) => {
  ...
  const eventFile = path.join(workerDir, 'events.jsonl');
  const metaFile = path.join(workerDir, 'worker.json');
  const eventRecord = (seq, data) => ({ v: 1, run: id, worker: path.relative(runDir, workerDir), attempt, seq, ts: 1, event: 'text_delta', data });
  const meta = { v: 1, run: id, worker: path.relative(runDir, workerDir), attempt, first_seq: 1, last_seq: 1, interactive: false };
  fs.writeFileSync(metaFile, JSON.stringify(meta));
  fs.writeFileSync(eventFile, JSON.stringify(eventRecord(1, 'initial transcript')) + '\n');
  ...
  const initialEvents = (await a.wait((e) => e.event === 'x-agentlinkd.crew.events' && e.data.events?.length)).data;
  assert.equal(initialEvents.events[0].data, 'initial transcript');
  ...
  fs.appendFileSync(eventFile, JSON.stringify(eventRecord(2, 'offline transcript')) + '\n');
  fs.writeFileSync(metaFile, JSON.stringify({ ...meta, last_seq: 2 }));
  const b = await connect();
  b.send("x-agentlinkd.crew.subscribe", { run: id, worker: "0", subscription: "reconnect", cursor: { generation: next.generation, offset: next.offset }, events_cursor: { attempt, seq: initialEvents.seq } });
  ...
  const eventReplay = (await b.wait((e) => e.event === 'x-agentlinkd.crew.events' && e.data.events?.length)).data;
  assert.equal(eventReplay.reset, false);
  assert.deepEqual(eventReplay.events.map((r) => r.data), ['offline transcript']);
  b.send('x-agentlinkd.crew.permission', { run: id, worker: '0', attempt: 'stale', id: 'p1', choice: 'once' });
  assert.equal((await b.wait((e) => e.event === 'x-agentlinkd.crew.permission_ack')).data.accepted, false);
