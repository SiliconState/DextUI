...
function toolFoldingPlan() {
  ...
  return [
    ...
    // Leave the completed read group on screen while Thinking remains active:
    // the browser regression opens it now and proves completion never closes it.
    { event: "thinking_delta", data: "Checking the activity hierarchy while preserving the user's disclosure choice.", delay: 700 },
    { event: "thinking_block_complete", data: "Checking the activity hierarchy while preserving the user's disclosure choice.", delay: 5 },
    ...
    { event: "tool_batch_end", data: { failed: 0 }, delay: 2 },
    { event: "text_block_complete", data: "Writing the generated presentation fixture.", delay: 3 },
    ...tool("fold-write", "write_file", "write_file: apps/web/src/generated.ts", "@@ -0,0 +1,2 @@\n+export const compact = true;\n+export const details = 'preserved';"),
    { event: "text_block_complete", data: "Checking the local endpoint response.", delay: 3 },
    ...tool("fold-http", "http", "http: GET https://example.test/health", "HTTP/1.1 200 OK\ncontent-type: application/json\n\n{\"ok\":true}"),
    { event: "text_block_complete", data: "Inspecting the generated image.", delay: 3 },
    ...tool("fold-image", "read_image", "read_image: artifacts/review.png", "approved image artifacts/review.png (800x600, sanitized as image/jpeg); pixels are available to the model only in this turn"),
    { event: "text_block_complete", data: "Recording the verified changes.", delay: 3 },
    ...tool("fold-commit", "git_commit", "git_commit: Fold tool activity", "[master abc1234] Fold tool activity\n 4 files changed, 40 insertions(+), 8 deletions(-)"),
