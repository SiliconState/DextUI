...
- Result assembly uses the final completed text block, not the event stream or intermediate narration, and discards aborted text before capturing a replacement stream. Existing file-based output, structured-output and acceptance contracts remain authoritative. Result/stdout and stderr assembly retain their existing 1 MiB caps; exceeding a result cap can still fail a worker. More than 8 MiB of tool-output events does not itself overflow final-result capture.
...
Structured replay uses `{attempt, seq}`. The host reads only complete records from the latest 512 KiB of the active event file, returns at most 256 records per tick, and reconciles every 250 ms. Changed attempts reset the projection; only events through the registration's published sequence are replayed. Malformed records flag a gap without repeatedly resetting pagination. Missing sequence ranges, rotation, malformed records and producer truncation are visible gaps. The archived segment is not replayed. Client state retains at most 200 blocks and approximately 512 KiB of serialized block content, rebuilding tool indexes after eviction. Unknown events are ignored by the existing session fold.

Pending approval metadata contains only the request ID, tool, bounded summary and choices, not potentially oversized tool arguments. It is sent even with an empty event replay, allowing recovery after a reconnect or retained-window gap. Registration control tokens and socket paths are not sent to the browser. The host validates run/worker membership and confines file access to regular, non-symlink files below the run. Socket backpressure pauses reads at the existing 256 KiB queued-output threshold; a single bounded replay chunk can exceed that threshold. There is no unbounded per-worker send queue.
...
- Web typecheck reports zero errors/warnings; all 203 root-suite tests and existing browser smoke pass. Browser smoke covers established UI behavior, not every worker-control interaction.
